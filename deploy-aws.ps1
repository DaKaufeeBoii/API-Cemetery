<#
.SYNOPSIS
    Deploys API Cemetery full stack (CloudFront, S3 Hosting, API Gateway, Lambda, DynamoDB) to AWS.
.DESCRIPTION
    1. Validates and builds SAM backend
    2. Deploys CloudFormation stack via SAM
    3. Builds Next.js frontend with exported static assets
    4. Syncs web app to S3 Hosting bucket
    5. Invalidates CloudFront distribution cache
    6. Displays live URLs
#>
...
# 2. Build SAM Backend
Write-Host "`n[2/6] Building AWS SAM backend..." -ForegroundColor Yellow
...
Write-Host "`n[3/6] Deploying CloudFormation stack (api-cemetery-prod)..." -ForegroundColor Yellow
sam deploy `
...
Write-Host "`n[4/6] Retrieving stack outputs..." -ForegroundColor Yellow
...
Set-Content -Path ".\.env.production" -Value "NEXT_PUBLIC_PROJECT_ID=proj-demo-001`nNEXT_PUBLIC_API_URL=$apiUrl"

# 5. Build Next.js Static Export
Write-Host "`n[5/6] Building Next.js static export..." -ForegroundColor Yellow
npm run build
if ($LASTEXITCODE -ne 0) {
...
Write-Host "`n[6/6] Syncing assets to S3 and invalidating CloudFront..." -ForegroundColor Yellow
